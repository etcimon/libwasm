...
+/
module libwasm.core.algebraic;

import libwasm.core.internal.meta;
import libwasm.core.functional: naryFun;
...
private static immutable variantExceptionMsg = "libwasm.core.algebraic: the algebraic stores other type then requested.";
private static immutable variantNullExceptionMsg = "libwasm.core.algebraic: the algebraic is empty and doesn't store any value.";
private static immutable variantMemberExceptionMsg = "libwasm.core.algebraic: the algebraic stores a type that isn't compatible with the user provided visitor and arguments.";
...
@safe pure version(mir_core_test) unittest
{
    import libwasm.core.functional: Tuple;
...
{
    import libwasm.core.internal.meta: getUDAs;
    import core.lifetime: moveEmplace;
    import libwasm.core.conv: emplaceRef;
    import libwasm.core.reflection: isPublic, hasField, isProperty;
    ...
    static foreach (i, T; MetaInfo__)
        mixin ((Vector!char("MetaInfo__[") << i.stringof << "].Type " << T.tag << ";")[]);
    ...
    private static union Storage__
    {
        ...
        static foreach (int i, P; _Payload)
            mixin((Vector!char(`alias _member_`) << i.stringof << ` = payload[` << i.stringof << `];`)[]);
                ...
                if (identifier__ == i)
                {
                    ...
                    else
                    {
                        storage__ = () inout @trusted {
                            mixin((Vector!char(`inout Storage__ ret = { _member_`) << i.stringof << ` : rhs.trustedGet!T };`)[]);
                            return ret;
                        } ();
                        return;
                    }
        ...
        else
        {
            ...
            this(return ref scope Algebraic rhs)
            {
                ...
                static if (!is(T == typeof(null)) && !is(T == void))
                {
                    ...
                    {
                        storage__ = () {
                            mixin((Vector!char(`Storage__ ret = { _member_`) << i.stringof << ` : rhs.trustedGet!T };`)[]);
                            return ret;
                        } ();
                        return;
                    }
            ...
            this(return ref scope const Algebraic rhs) const
            {
                ...
                static if (!is(T == typeof(null)) && !is(T == void))
                {
                    ...
                    {
                        storage__ = () const {
                            mixin((Vector!char(`const Storage__ ret = { _member_`) << i.stringof << ` : rhs.trustedGet!T };`)[]);
                            return ret;
                        } ();
                        return;
                    }
            ...
            this(return ref scope immutable Algebraic rhs) immutable
            {
                ...
                static if (!is(T == typeof(null)) && !is(T == void))
                {
                    ...
                    {
                        storage__ = () immutable {
                            mixin((Vector!char(`immutable Storage__ ret = { _member_`) << i.stringof << ` : rhs.trustedGet!T };`)[]);
                            return ret;
                        } ();
                        return;
                    }
        ...
        switch (identifier__)
        {
            ...
            static foreach (i, T; AllowedTypes)
            {
                case i: {
                    ...
                    {
                        debug pragma(msg, (Vector!char("Mir warning: couldn't compute hash. Expected a `size_t toHash() scope @safe const pure nothrow @nogc` method for ") << T.stringof)[]);
                        hash = i;
                    }
                    break S;
                }
        ...
        switch (identifier__)
        {
            ...
            static foreach (i, T; AllowedTypes)
            {
                case i: {
                    ...
                    {
                        debug pragma(msg, (Vector!char("Mir warning: can't compute hash. Expexted `size_t toHash() scope @safe const pure nothrow @nogc` method for ") << T.stringof)[]);
                        hash = i;
                    }
                    break S;
                }
        ...
        if (__traits(compiles, ()pure{ w.put("Algebraic"); }))
    {
        ...
        {
            import libwasm.core.format: print;
            static foreach (i, member; metaFieldNames__)
            static if (!MetaInfo__[i].transparent)
            {
                static if (__traits(compiles, { import libwasm.core.format: print; print(w, __traits(getMember, this, member)); }))
                    { import libwasm.core.format: print; print(w, __traits(getMember, this, member)); }
                else
            ...
            switch (identifier__)
            {
                static foreach (i, T; AllowedTypes)
                {
                    case i:
                        ...
                        static if (__traits(compiles, { import libwasm.core.format: print; print(w, trustedGet!T); }))
                            toStringImpl!T(w);
                        else
                            w.put(AllowedTypes[i].stringof);
                        return;
    ...
    static if (is(AllowedTypes[0] == typeof(null)))
    {
        ...
        {/*
            import libwasm.core.utility: _expect;
            if (_expect(!identifier__, false))
            {
                assert(false, "Null Variant Exception");
            }*/
        ...
        static if (AllowedTypes.length == 2)
        {
            ...
            +/
            pure auto ref inout(AllowedTypes[1]) get() return inout
            {
    ...
    +/
    pure auto ref trustedGet(R : Algebraic!RetTypes, this This, RetTypes...)() return @property
        if (allSatisfy!(Contains!AllowedTypes, Algebraic!RetTypes.AllowedTypes))
    {
        static if (is(RetTypes == Types__))
            return this;
    ...
    {
        ///
        pure auto ref trustedGet(this This)() return
        {
    ...
    +/
    pure auto ref get(R : Algebraic!RetTypes, this This, RetTypes...)() return @property
        if (allSatisfy!(Contains!AllowedTypes, Algebraic!RetTypes.AllowedTypes))
    {
        static if (is(RetTypes == Types__))
            return this;
    ...
    {
        ///
        pure auto ref get(this This)() return
        {
    ...
    static if (typeFieldNames__.length)
    {
        ...
        static foreach (member; typeFieldNames__)
            mixin ((Vector!char("alias ") << member << `() = get!"` << member << `";`)[]);
    }
        ...
        if (Args.length && (Args.length > 1 || !isLikeVariant!(Args[0])))
    {
        ...
        static if (TargetType.length == 0)
            static assert(0, (Vector!char() << typeof(this).stringof << ".this: no types can be constructed with arguments " << Args.stringof)[]);
        static assert(TargetType.length == 1, (Vector!char() << typeof(this).stringof << ".this: multiple types " << TargetType.stringof << " can be constructed with arguments " << Args.stringof)[]);
        alias TT = TargetType[0];
        static if (is(TT == struct) || is(TT == union))
            this(CopyTypeQualifiers!(This, TT)(forward!args));
        else
            this(alloc!(CopyTypeQualifiers!(This, TT))(forward!args));
    }
    ...
    static if (_ReflectionTypes.length && allSatisfy!(isSimpleAggregateType, _ReflectionTypes))
    {
        ...
        static if (allSatisfy!(ApplyRight!(isPublic, member), _ReflectionTypes))
        {
            ...
            {
                mixin((Vector!char(`ref `) << member << q{()() inout return @trusted pure nothrow @nogc @property { return this.getMember!member; }})[]);
            }
            else
            static if (allSatisfy!(ApplyRight!(templateOr!(hasField, isProperty), member), _ReflectionTypes))
            {
                mixin((Vector!char(`auto ref `) << member << q{(this This, Args...)(auto ref Args args) @property { static if (args.length) { import core.lifetime: forward; return this.getMember!member = forward!args; } else return this.getMember!member;  }})[]);
            }
            static if (allSatisfy!(ApplyRight!(templateNot!(templateOr!(hasField, isProperty)), member), _ReflectionTypes))
            {
                mixin((Vector!char(`template `) << member << `(TArgs...) { auto ref ` << member << q{(this This, Args...)(auto ref Args args) { static if (args.length) { import core.lifetime: forward; return this.getMember!(member, TArgs)(forward!args); } else return this.getMember!(member, TArgs);  }} << `}`)[]);
            }
        }
    }
    ...
    static foreach (int i, T; AllowedTypes)
    {
        ...
        pure auto ref trustedGet(E)() @trusted @property return inout nothrow
            if (is(E == T))
        {
        ...
        +/
        pure auto ref get(E)() @property return inout
            if (is(E == T))
        {
            import libwasm.core.utility: _expect;
            static if (AllowedTypes.length > 1)
            {
                ...
                {
                    assert(false, "Wrong type returned");
                }
            }
            return trustedGet!T;
        }
        ...
        static if (is(T == void))
        {
            ...
            static Algebraic _void()
            {
                ...
                ret.storage__ = () {
                    import core.lifetime: forward;
                    mixin((Vector!char(`Storage__ ret = { _member_`) << i.stringof << ` : _Void!().init };`)[]);
                    return ret;
                } ();
                ret.identifier__ = i;
                return ret;
        ...
        else
        {
            ...
            this(T value)
            {
                ...
                {
                    storage__ = () {
                        mixin((Vector!char(`Storage__ ret = { _member_`) << i.stringof << ` : rhs };`)[]);
                        return ret;
                    } ();
                }
            ...
            this(const T value) const
            {
                ...
                {
                    storage__ = () {
                        mixin((Vector!char(`const Storage__ ret = { _member_`) << i.stringof << ` : rhs };`)[]);
                        return ret;
                    } ();
                }
                static if (_Payload.length > 1)
            ...
            this(immutable T value) immutable
            {
                ...
                {
                    storage__ = () {
                        mixin((Vector!char(`immutable Storage__ ret = { _member_`) << i.stringof << ` : rhs };`)[]);
                        return ret;
                    } ();
                }
                static if (_Payload.length > 1)
...
unittest
{
    ...
    @safe pure nothrow @nogc:
        ...
        size_t toHash() scope const { return hashOf(_b) ^ a; }
...
+/
@safe nothrow version(mir_core_test) unittest
{
    import libwasm.core.conv: to;
    enum MIR_ALGORITHM = __traits(compiles, { import libwasm.core.format; });
...
unittest
{
    ...
    import libwasm.core.utility: min;
...
*/
    ...
    else
    {
        ...
        static assert(is(typeof(collide(ea, es)) == void), typeof(collide(ea, es)).stringof);
    }
...
{
    ///
    pure auto ref getMemberHandler(V, Args...)(ref V value, auto ref Args args)
    {
        ...
        {
            import core.lifetime: forward;
            import libwasm.core.reflection: hasField;
            static if (TArgs.length)
            {
...
private template visitLast(alias visitor)
{
    template visitLast(T)
    {
        static if (is(T == void))
        {
            pure auto ref visitLast(Args...)(auto ref Args args)
            {
                import core.lifetime: forward;
...
{
    ///
    pure auto ref visitImpl(Args...)(auto ref Args args)
        if (Filter!Args)
    {
...
{
    import libwasm.memory.scoped: copy;
    auto r = Vector!char("enum Kind {");
    foreach (s; strs)
    {
        r ~= s;
        r ~= ", ";
