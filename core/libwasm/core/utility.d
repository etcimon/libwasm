...
+/
module libwasm.core.utility;

import std.traits;

import libwasm.core.math.common: optmath;
...
void swapStars(I1, I2)(auto ref I1 i1, auto ref I2 i2)
{
    ...
    {
        import libwasm.core.functional: unref;
        auto e = unref(*i1);
        i1[0] = *i2;
        i2[0] = e;
    }
    ...
    if (T.length >= 2)
{
    ...
    {
        import libwasm.core.math.common: fmin;
        return fmin(a, b);
    }
    else
    {
        static if (isIntegral!T0 && isIntegral!T1)
            static assert(isSigned!T0 == isSigned!T1, 
                "libwasm.core.utility.min is not defined for signed + unsigned pairs because of security reasons."
    ...
    if (T.length >= 2)
{
    ...
    static if ((isFloatingPoint!T0 && isNumeric!T1) || (isFloatingPoint!T1 && isNumeric!T0))
    {
        import libwasm.core.math.common: fmax;
        return fmax(a, b);
    }
    else
    {
        static if (isIntegral!T0 && isIntegral!T1)
            static assert(isSigned!T0 == isSigned!T1, 
                "libwasm.core.utility.max is not defined for signed + unsigned pairs because of security reasons."
                ~ "Please unify type or use a Phobos analog.");
