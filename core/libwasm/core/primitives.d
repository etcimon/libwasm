...
Note:
UTF strings behaves like common arrays in libwasm.core.
...
+/
module libwasm.core.primitives;

import libwasm.core.internal.utility;
...
{
    import libwasm.core.ndslice.slice: Slice, SliceKind;
...
package(libwasm) bool anyEmptyShape(size_t N)(scope const auto ref size_t[N] shape) @property
{
    foreach (i; Iota!N)
        if (shape[i] == 0)
            return true;
