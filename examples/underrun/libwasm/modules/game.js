import {libwasm, encoders, decoders} from './libwasm.js';
...
export let jsExports = {
    env:{
        ...
        load_image(len, offset, ctx, func) {
            let img = new Image();
            img.src = 'images/'+decoders.string(len,offset)+'.png';
            img.onload = function(){
                let ptr = addObject(this);
                libwasm.instance.exports.jsCallback(ctx,func,ptr);
            };
        },
        load_level(id, bufLen, bufOffset, ctx, func) {
            let img = new Image();
            let level = new Uint8Array(libwasm.memory.buffer, bufOffset, 64*64);
            ...
            img.onload = function(){
                ...
                libwasm.instance.exports.jsCallback(ctx,func,bufOffset);
            };
        },
        scheduleFrame(ctx, func) {
            requestAnimationFrame(()=>{
                let time = parseInt(performance.now(),10);
                libwasm.instance.exports.jsCallback(ctx, func, time);
            })
        }
    }
}
