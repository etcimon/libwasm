...
export let jsExports = {
  env: {
    CanvasText_fillText_0: (ctx, textLen, textPtr, x, y) => {
      setupMemory();
      libwasm.objects[ctx].fillText(libwasm_decode_string(textLen, textPtr), x, y);
    },
    HTMLCanvasElement_width_Set: (ctx, width) => {
      setupMemory();
      libwasm.objects[ctx].width = width;
    },
    HTMLCanvasElement_height_Set: (ctx, height) => {
      setupMemory();
      libwasm.objects[ctx].height = height;
    },
    ...
    Document_createElement_0: (ctx, localNameLen, localNamePtr) => {
      setupMemory();
      return libwasm.addObject(libwasm.objects[ctx].createElement(libwasm_decode_string(localNameLen, localNamePtr)));
    },
    ...
    ParentNode_querySelector: (rawResult, ctx, selectorsLen, selectorsPtr) => {
      setupMemory();
      libwasm_encode_optional_Handle(rawResult, libwasm.objects[ctx].querySelector(libwasm_decode_string(selectorsLen, selectorsPtr)));
    },
  }
}