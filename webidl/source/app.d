...
void main(string[] args)
{
  ...
  if (generateBindings) {
    if (jsFolder == "")
      jsFolder = getcwd() ~ "/libwasm/modules";
...
void generateDFiles(IR ir, string dFolder) {
  ...
  std.file.write(
                 ...
                 chain(
                       ...
                       ir.semantics.modules.values.map!(m => format("public import libwasm.bindings.%s;",m.name)).joiner("\n")
                       ).text()
                 );
}
...
void generateJsFiles(IR ir, string jsFolder, string[] jsExportFilters) {
  ...
  std.file.write(jsFolder ~ "/index.js", chain(only("// File is autogenerated with `dub run libwasm:webidl -- --bindgen`"), imports, only(modulesLine), only("export { libwasm, modules }")).joiner("\n").text);
}
