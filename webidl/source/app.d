...
void generateJsFiles(IR ir, string jsFolder, string[] jsExportFilters) {
  ...
  std.file.write(jsFolder ~ "/index.js", chain(only("// File is autogenerated with `dub run spasm:webidl -- --bindgen`"), imports, only(modulesLine), only("export { spasm, modules }")).joiner("\n").text);
}
