...
// version (CRuntime_LIBWASM) {} // Removed the GC from here

private extern (C) void* _d_allocmemory(size_t sz) pure nothrow;
// emplace
/**
    ...
    if (is(T : Throwable) && __traits(getLinkage, T) == "D")
{
    ...
    void* p = _d_allocmemory(init.length);
    if (!p)
    {
...
T* _d_newitemT(T)() @trusted
{
    ...
    auto p = _d_allocmemory(totalSize);
