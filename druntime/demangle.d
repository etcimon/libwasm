...
pure @safe nothrow:
    ...
    void silent( void delegate() pure @safe nothrow dg )
    {
    ...
    */
    char[] parseType( char[] name = null ) return scope
    {
        ...
        char[] parseBackrefType(scope char[] delegate() pure @safe nothrow parseDg) pure @safe
        {
            if (pos == brp)
...
//        printf( "*** %c\n", front );
    ...
    */
    ...
    bool mayBeMangledNameArg()
    {
        ...
        {
            auto ret = pos < buf.length && '_' == buf[pos++] &&
                   pos < buf.length && 'D' == buf[pos++] &&
                   isSymbolNameFront();
            pos = p;
            return ret;
