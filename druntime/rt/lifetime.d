 ...
 */
extern (C) void* _d_allocmemoryT(TypeInfo ti) pure nothrow
{
    return _d_allocmemory(ti.tsize());
}
...
private extern (D) Object _d_newclass(bool initialize)(const ClassInfo ci)
{
    ...
    else
    {
        ...
        p = _d_allocmemory(init.length);//, attr, ci);
...
private BlkInfo __arrayAlloc(size_t arrsize, const scope TypeInfo ti, const TypeInfo tinext) nothrow pure
{
    ...
    auto bi = _d_allocmemory(padded_size);
...
@property BlkInfo *__blkcache() nothrow
{
    if (!__blkcache_storage)
    {
        ...
        __blkcache_storage = cast(BlkInfo *)_d_allocmemory(size);
...
extern(C) void _d_arrayshrinkfit(const TypeInfo ti, void[] arr) nothrow
{
    ...
    auto info = bic ? *bic : null;
    if (info && info.base && (info.attr & BlkAttr.APPENDABLE))
    {
...
Lcontinue:
    ...
    if (info && info.size >= PAGESIZE && curcapacity != 0)
    {
