...
enum
{ // overhead allocator definitions, lazily loaded
...
version (DictionaryDebugger)
	const HasDictionaryDebugger = true;
else
	const HasDictionaryDebugger = false;
version (EnableDebugger)
	const HasDebuggerEnabled = true;
else
	const HasDebuggerEnabled = false;
version (DisableDebugger)
	const DisableDebugAllocations = true;
else version (VibeNoDebug)
	const DisableDebugAllocations = true;
else
	const DisableDebugAllocations = false;
	public:
	static if (HasDebuggerEnabled && !DisableDebugAllocations)
	const HasDebugAllocations = true;
else static if (!DisableDebugAllocations)
	const HasDebugAllocations = true;
else
	const HasDebugAllocations = false;
package:
version (SkipMemutilsTests)
	const SkipUnitTests = true;
else
	const SkipUnitTests = false;
