...
- `libwasm.bridge`: `callTs` / `callTsPromise` call `window.__svelteD.ts` through Lodash `invoke` (optional args keep JS defaults). `setDRet` is the D→TS return slot `callNative` reads. `exportDelegate` / `callNative` stay the JS→D seam.
