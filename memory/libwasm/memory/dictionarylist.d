...
*/
module libwasm.memory.dictionarylist;
...
alias DictionaryListRef(KEY, VALUE, ALLOC = AppMem, bool case_sensitive = true, size_t NUM_STATIC_FIELDS = 8) = RefCounted!(DictionaryList!(KEY, VALUE, ALLOC, case_sensitive, NUM_STATIC_FIELDS), ALLOC);
...
*/
struct DictionaryList(KEY, VALUE, ALLOC = AppMem, bool case_sensitive = true, size_t NUM_STATIC_FIELDS = 8) {
nothrow @nogc @trusted:
	@disable this(this);
    ...
    */
	...
	/// ditto
	void getValuesAt(in KeyType key, scope void delegate(const(ValueType)) nothrow @nogc del)
	const {
    ...
    */
	...
	inout {
		...
		static if (isSomeString!ValueType) {
			if (pitm is null) return ValueType("?");
		} 
		else{
			if (pitm is null) return ValueType.init;
		}
		return *pitm;
	}
    ...
    */
	int opApply(int delegate(KeyType key, ref ValueType val) nothrow @nogc del)
	{
	...
	int opApply(int delegate(const ref KeyType key, const ref ValueType val) nothrow @nogc del) const
	{
		foreach (ref kv; m_fields[0 .. m_fieldCount]) {
			if (kv == Field.init) return 0;
			if (auto ret = del(kv.key, kv.value))
	...
	/// ditto
	int opApply(int delegate(ref ValueType val) nothrow @nogc del)
	{
		foreach (KeyType key, ref ValueType val; this) {
			if (auto ret = del(val))
				return ret;
		}
		return 0;
	...
	/// ditto
	/*
	int opApply(int delegate(KeyType key, ref const(ValueType) val) @nogc nothrow del) const
	{
		return (cast() this).opApply(cast(int delegate(KeyType, ref ValueType) @nogc nothrow) del);
	}*/
	
	/// ditto
	int opApply(int delegate(ref const(ValueType) val) @nogc nothrow del) const
	{
		return (cast() this).opApply(cast(int delegate(ref ValueType) @nogc nothrow) del);
	}
