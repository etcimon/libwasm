...
*/
module libwasm.memory.freelist;

import libwasm.memory.allocators;
import libwasm.memory.memory;
import libwasm.memory.helpers : destructRecurse, min, memset;
import libwasm.memory.utils : ObjectAllocator, Malloc;
...
nothrow @nogc @trusted:
	...
	{
		logTrace("{0} {1}", "AFL free", data.length);
		foreach(i; iotaTuple!freeListCount) {
