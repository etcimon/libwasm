module libwasm.memory.utils;

import libwasm.memory.allocators;
import libwasm.memory.constants;
import libwasm.memory.vector : Array;
import libwasm.memory.helpers : UnConst, memset, memcpy;
import libwasm.memory.memory;
...
struct AppMem {
	nothrow:
	@trusted:
	mixin ConvenienceAllocators!(LocklessFreeList, AppMem);
}

struct AppMemAllocator {
static:
nothrow @nogc @trusted:
    void[] allocate(size_t n) {
        return AppMem.alloc!(void[])(n);
    }

    void[] reallocate(void[] data, size_t n, bool free_mem = true) {
        if (data.length == 0)
            return allocate(n);
        return AppMem.realloc(data, n, free_mem);
    }

    bool deallocate(void[] data, bool free_mem = true) {
        AppMem.free(data, free_mem);
        return true;
    }
}
...
nothrow @nogc @trusted:
	...
	import libwasm.memory.scoped : PoolStack;
    void[] allocate(size_t n) {
...
nothrow @trusted extern(C) {
    void[] FL_allocate(size_t n) {
        return AppMemAllocator.allocate(n);
    }
    void[] FL_reallocate(void[] mem, size_t n){
        return AppMemAllocator.reallocate(mem, n);
    }
    void FL_deallocate(void[] mem){
        AppMemAllocator.deallocate(mem);
    }
}
...
nothrow:
	...
	void free(TR obj)
	{
		...
		TR objc = obj;
		import libwasm.memory.helpers : destructRecurse;
...
struct ObjectAllocator(T, ALLOC = AppMem)
{
static nothrow @nogc @trusted:
	...
	TR alloc(ARGS...)(auto ref ARGS args)
	{
		if (__ctfe) {
			...
			static if (ALLOC.stringof == "PoolStack") {
				...
				auto mem = AppMem.alloc!(void[])(ElemSize);
			} else {
	...
	void free(TR obj)
	{
		...
		if (!__ctfe) {
			...
			import libwasm.memory.helpers : destructRecurse;
			static if (is(TR == T*) && hasElaborateDestructor!T) {
				logTrace("ObjectAllocator.free Pointer destr {0}", T.stringof);
				destructRecurse(*objc);
			} 
			else static if (hasElaborateDestructor!TR) {
				logTrace("ObjectAllocator.free other destr {0}", T.stringof);
				destructRecurse(objc);
			} 
...
T[] allocArray(T, ALLOC = AppMem)(size_t n, ALLOC* base = null)
{
	...
	if (__ctfe) {
		...
		static if (ALLOC.stringof == "void") {
			auto mem = AppMem.alloc!(void[])(TSize * n);
...
T[] reallocArray(T, ALLOC = AppMem)(T[] array, size_t n, ALLOC* base = null) {
	static enum TSize = T.sizeof;
...
nothrow void freeArray(T, ALLOC = AppMem)(auto ref T[] array, size_t max_destroy = size_t.max, size_t offset = 0, ALLOC* base = null)
{
	static enum TSize = T.sizeof;
	...
	else static if (ALLOC.stringof != "CTFE") {
		static if (ALLOC.stringof == "PoolStack")
			auto allocator = ALLOC.top;
		else static if (hasMember!(ALLOC, "ident")) {
