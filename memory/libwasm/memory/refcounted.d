module libwasm.memory.refcounted;
...
struct RefCounted(T, ALLOC = AppMem)
{
...
nothrow @nogc @trusted:
	...
	static void dtor(U)(U* ctxt) {
		logTrace("Call dtor {0} for {1}", U.stringof, typeof(this).stringof);
		static if (!is (U == typeof(this))) {
			logTrace("U != typeof(this)");
	...
	private void opAssignImpl(U)(U other) {
		...
		if( m_object ) {
			(*m_refCount)++;
			logTrace("Incr: {0} = {1}", U.stringof, *m_refCount);
		}
	}
	...
	private void _clear()
	{
		...
		logTrace("Clear: {0} = {1}", T.stringof, m_object ? *m_refCount : -1);
		//checkInvariants();
		if( m_object ){
			if( --(*m_refCount) == 0 ){
				if (m_free)
	...
	{
		logTrace("RefCounted opcast: bool {0}", T.stringof);
