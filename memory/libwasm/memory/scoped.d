...
package:
version(unittest) {
	import libwasm.memory.debugger;
	alias Pool = PoolAllocator!(DebugAllocator!(AutoFreeListAllocator!(MallocAllocator)));
}
else {	
...
nothrow @nogc @trusted:
	...
	void push(ref Vector!(ManagedPool, AppMem) pools)
	{
	...
	Vector!(ManagedPool, AppMem) pop(size_t n) {
		assert(!empty);
		//logTrace("Pop Thread Freezer of ", m_pools.length, " id ", m_pools.back.id);
		// already sorted
		auto pools = Vector!(ManagedPool, AppMem)( m_pools[$-n .. $] );
...
package:
	Vector!(ManagedPool, AppMem) m_pools;
}
...
nothrow @nogc @trusted:
	...
	ManagedPool opIndex(size_t n) {
		logTrace("OpIndex[{0}] in Thread Pool of {1} top: {2} id: {3}", n, length,  cnt, m_pools[n].id);
		return m_pools[n];
	}
	...
	Vector!(ManagedPool, AppMem) freeze(size_t n) {
		//assert(!empty);
		...
		//assert(n <= length);
		Vector!(ManagedPool, AppMem) ret = Vector!(ManagedPool, AppMem)( m_pools[$-n .. $] );
		m_pools.length = (m_pools.length - n);
	...
	void unfreeze(ref Vector!(ManagedPool, AppMem) pools) {
