...
*/
module libwasm.memory.circularbuffer;
...
struct CircularBuffer(T, size_t N = 0, ALLOC = void) {
	...
	static if( N == 0 ){
		...
		~this() { if (m_buffer.length > 0) freeArray!(T, ALLOC)(m_buffer, m_fill, m_start); m_buffer = null; }
	} 
	else {
	...
	inout(T)[] peek() inout { return m_buffer[m_start .. min(cast(size_t)(m_start+m_fill), m_buffer.length)]; }
	T[] peekDst() {
	...
	int opApply(scope int delegate(ref T itm) nothrow del)
	{
		if( m_start+m_fill > m_buffer.length ){
...
unittest {
	...
	test_numbers << 6 << 7 << 8;
	buf.put(test_numbers[]); // 6 7 8|4 5
	assert(buf.length == 5 && buf.freeSpace == 0);
	int[5] dst;
	buf.read(dst); // . . .|. .
	test_numbers.clear();
	test_numbers << 4 << 5 << 6 << 7 << 8;
	assert(dst == test_numbers[]);
	assert(buf.length == 0 && buf.freeSpace == 5);
	test_numbers.clear();
	test_numbers << 1 << 2;
	buf.put(test_numbers[]); // . . .|1 2
	assert(buf.length == 2 && buf.freeSpace == 3);
	buf.read(dst[0 .. 2]); //|. . . . .
	
	assert(dst[0 .. 2] == test_numbers[]);
}
