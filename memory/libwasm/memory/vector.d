module libwasm.memory.vector;

import std.algorithm : swap;
import std.traits : ForeachType, isNumeric, isSomeString, hasElaborateDestructor, isPointer, hasIndirections, Unqual;
...
nothrow @nogc @trusted:
	...
	private struct Payload
	{
		...
		{
			logInfo("Set length: {0}", newLength);
				...
				static if (hasElaborateDestructor!T) {
					foreach (ref e; _payload[newLength .. _length]) {
						static if (is(T == struct) && !isPointer!T) { // call destructors but not for indirections...
							logTrace("Destructing Vector item(s) {0} for newLength {1}", T.stringof, newLength);
							destructRecurse(e);
						}
					}
		...
		void reserve(size_t elements)
		{
			...
			logTrace("Reserve {0} => {1} elements with capacity {2}", length, elements, capacity);
	...
	*/
	...
	/// ditto
	@property RefCounted!(Vector!(T, ALLOC), AppMem) dupr() const
	{
		return RefCounted!(Vector!(T, ALLOC), AppMem)(cast(T[])_data._payload[0 .. _data._length]);
	}
	...
	*/
	...
	{
		logTrace("Slicing Vector i={0} j={1}", i, j);
