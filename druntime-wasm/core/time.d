...
@safe:
    ...
    static @property MonoTimeImpl currTime() @trusted nothrow @nogc
    {
        ...
        else version (Posix)
        {
            ...
            time_result_t timestamp_res = clock_time_get(CLOCK_REALTIME, -1);
            if (timestamp_res.errno)
            {
                import core.internal.abort : abort;
                abort("Call to clock_time_get failed.");
            }
            return MonoTimeImpl(timestamp_res.timestamp);
        }
    }
...
deprecated:
    ...
    static @property TickDuration currSystemTick() @trusted nothrow @nogc
    {
        ...
        version (WASI) {
            ...
            return TickDuration(timestamp_res.timestamp);
        }
        else version (Windows)
        {
            ulong ticks = void;
