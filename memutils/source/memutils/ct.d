...
{
    static if (is(typeof(&func[0]) Fsym : Fsym*) && is(Fsym == function) || is(
            typeof(&func[0]) Fsym == delegate))
    {
...
template ParameterIdentifierTuple(func...) if (func.length == 1 && isCallable!func)
{
    ...
    static if (is(FunctionTypeOf!func PT == __parameters))
    {
        ...
        {
            static if (!isFunctionPointer!func && !isDelegate!func // Unnamed parameters yield CT error.
...
version (none)  : template Tuple(Specs...) if (distinctFieldNames!(Specs))
{
    import std.meta : AliasSeq;
    ...
    enum areCompatibleTuples(Tup1, Tup2, string op) = isTuple!Tup2 && is(
            typeof(
    ...
    struct Tuple
    {
         ...
         */
        auto opBinary(string op, T)(auto ref T t) if (op == "~")
        {
            static if (isTuple!T)
            {
        ...
        /// ditto
        auto opBinaryRight(string op, T)(auto ref T t) if (op == "~")
        {
            static if (isTuple!T)
            {
                static assert(distinctFieldNames!(_Fields, T._Fields),
