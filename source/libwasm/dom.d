    ...
    void toString(scope void delegate(const(char)[]) @safe sink) @trusted {
      ...
      if (type != NodeType.root) {
        ...
        foreach (kv; properties.byKeyValue()) {
          sink.formattedWrite(" %s=%s", kv.key, libwasm.sumtype.match!((string s)=>format(`"%s"`,s),(i)=>i.to!string)(kv.value));
        }
        foreach (kv; attributes.byKeyValue()) {
          sink.formattedWrite(" %s=%s", kv.key, libwasm.sumtype.match!((string s)=>format(`"%s"`,s),(int i)=>i.to!string)(kv.value));
        }
        sink(">");
      }
...
@trusted
void setParamFromParent(string injected_name, string local_name, T, Ts...)(ref T t, auto ref Ts ts) {
...
void setChildFromParent(string injected_name, string local_name, T, Ts...)(auto ref T t, auto ref Ts ts) {
  ...
  static if (index != -1) {
    ...
    import libwasm.bindings.html : HTMLElement;
...
void verifyChildParams(Super, string parentField, Params...)() {
  ...
  static foreach(Param; AliasSeq!ParamsTuple) {{
      ...
      static if (!isBasicType!(ChildMemberType) && !isValue!(Param.Field) && hasMember!(Super, childName)) {
        ...
        static assert(!hasUDA!(__traits(getMember, Super, childName), child), "Injected member '"~ childName~ "' cannot have @child attribute in struct "~ Super.stringof);
        
        // To make sure we don't inject an injected field
        // Todo: Omit @inject fields from the tuple
        static assert(childOffset > parentOffset, "Injected member '"~ childName~ "' needs to be declared after '"~parentField~ "' in struct "~ Super.stringof);
      }
    }}
}
  ...
  static foreach(sym; T.tupleof) {{
      ...
      static if (!isImmutable) {
        ...
        static if (is(ChildType : NamedNode!name, string name)) {
          static if (i != "node") { // reference to a parent struct
          ...
          static if (isPublic) { // reference to a parent member   
          ...
          static if (isPublic && !is(sym) && isAggregateType!T) {
            ...
            static if (is(typeof(sym) : DynamicArray!(Item), Item)) {
              ...
              static if (!isCallable!(typeof(sym))) {//} && !isPointer!(typeof(sym))) {
                ...
                static if (isAggregateType!(ChildType)) { // recurse through the child members
                  static if (isPointer!(typeof(sym)))
...
auto callMember(string fun, T)(auto ref T t) {
  import libwasm.ct : ParameterIdentifierTuple;
...
template updateChildren(alias member) {
  ...
  static void updateChildren(Parent)(auto ref Parent parent) {
    ...
    static foreach(c; children) {{
        ...
        static if (hasMemberWithInjectOf!(field, c)) {
