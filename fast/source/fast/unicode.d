 ...
 *
 **************************************/
@nogc @trusted pure nothrow size_t countGraphemes(scope const(char)[] str)
{
	...
	@noinline @safe @nogc pure nothrow bool complexRules()
	{
		pragma(inline, false);
		with (GraphemeBreakProperty.Enum)
		{
	...
	@forceinline void graphemeCountImpl(S)(ref S str)
	{
 ...
 * 
 **************************************/
@property @safe @nogc pure nothrow CodePointInfo!GeneralCategory generalCategory(
...
getProperty(Property, S)(scope S str)
		if (is(S == const(char)*) || is(S == const(char)[]))
in
{
	static if (is(S == const(char)[]))
